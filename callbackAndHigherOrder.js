...
contains(names, 'Tyler', result => {
  if(result === true){
    console.log('Tyler is in the array')
  } else {
    console.log('Tyler is not in the array')
  }
})
...
const getUserById = (arr, id, cb) => arr.forEach(elm.id === id ? cb(elem) :
...
// CODE HERE
const adddingFactory = x => y => x + y

const adddingFactory = (x) => (y) => x + y;


const adddingFactory = x => {
  return function(y) {
    return x + y 
  }
}
...
// CODE HERE
const addTen = adddingFactory(10)
/*
...
// CODE HERE
const addFour = adddingFactory(4)
